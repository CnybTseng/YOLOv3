...
class YOLOv3Decoder(torch.nn.Module):
    def __init__(self, in_size, num_classes, anchors):
        ...
        self.anchor_masks = self.LongTensor(((6,7,8),(3,4,5),(0,1,2)))
    ...
    def __decoder(self, x, anchors):
        ...
        bx = (x[..., 0].data + gx.data) / w
        by = (x[..., 1].data + gy.data) / h
        
        aw = anchors[:, 0].view(1, num_anchors, 1, 1)
        ah = anchors[:, 1].view(1, num_anchors, 1, 1)        
        bw = torch.exp(x[..., 2].data) * aw.data / self.in_size[1]
        bh = torch.exp(x[..., 3].data) * ah.data / self.in_size[0]
...
class YOLOv3EvalDecoder(YOLOv3Decoder):
    ...
    def forward(self, xs):
        ...
        cpros = [dx[1][...,5:].view(dx[1].size(0), -1, self.num_classes) for dx in dxs]
        outputs = [torch.cat((b,o,c), dim=-1) for (b,o,c) in zip(boxes,objes,cpros)]
...
if __name__ == '__main__':
    ...
    yolo1_in = torch.rand(8, 75, 13, 13).to(device)
    yolo2_in = torch.rand(8, 75, 26, 26).to(device)
    yolo3_in = torch.rand(8, 75, 52, 52).to(device)
    xs = [yolo1_in, yolo2_in, yolo3_in]
    anchors = ((10,13),(16,30),(33,23),(30,61),(62,45),(59,119),(116,90),(156,198),(373,326))
    
    targets = torch.zeros(size=(32,6), dtype=torch.float32)
    
    total_objs = np.sort(np.random.randint(low=0, high=33, size=(7,)))
    total_objs = np.append(total_objs, np.array([32]))
    
    print(f'total_objs={total_objs}')
    last_objs = 0
    for image_id, num_objs in enumerate(total_objs):
        targets[last_objs:num_objs, 0] = image_id
        last_objs = num_objs
    
    targets[:,1] = torch.randint(low=0, high=20, size=(32,)).type(FloatTensor)
    
    def get_rand_border():
        left = torch.randint(low=0, high=384, size=(32,))
        right = torch.zeros(size=(32,), dtype=torch.int32)
        for i, xmin in enumerate(left):
            right[i] = torch.randint(low=xmin.item(), high=416, size=(1,))
        return left.type(FloatTensor)/416, right.type(FloatTensor)/416
    
    left, right = get_rand_border()
    top, bottom = get_rand_border()
    targets[:,2], targets[:,3] = (left + right)/2, (top + bottom)/2
    targets[:,4], targets[:,5] = right - left, bottom - top
    targets = targets.to(device)
    
    print(f'targets=\n{targets}')
    loss = YOLOv3Loss(20, anchors)
    loss, metrics = loss(xs, targets, (416,416))
    print(f'loss={loss}')
    for metric in metrics: print(f'metric=\n{metric}')